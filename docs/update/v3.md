...
* 修复反向 WS 鉴权逻辑反转（无 token 会被放行）、BotMap 连接关闭时误注销同端口所有机器人等问题
* 修复 Schedule 排程任务共享 next_run 导致任务互相拖拽、异常后 no_overlap 永久失效的问题
* 修复 KVRedis clear 因 Redis DEL 不支持通配符而静默失效、LightCache 多进程模式下半初始化的问题
* 修复 PluginMeta 插件入口文件读取不存在的属性导致 extra.zm-plugin-main 配置永不生效的问题
* 修复三个数据库驱动读取错误配置键、DBWrapper 双重关闭连接、调试日志残留等问题
* 统一 workerman 驱动 worker 数量配置键，并兼容旧键名
* 新增以上问题的回归测试，删除引用 2.x 废弃类的 tests_old 目录

**源码变更记录**: <https://github.com/zhamao-robot/zhamao-framework/compare/3.2.9...3.2.10>

## v3.2.9

> 更新时间：2026-07-24

**源码变更记录**: <https://github.com/zhamao-robot/zhamao-framework/compare/3.2.8...3.2.9>
